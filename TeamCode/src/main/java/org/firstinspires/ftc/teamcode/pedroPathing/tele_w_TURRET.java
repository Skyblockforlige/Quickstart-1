    ...
    public void runOpMode() {
        ...
        Thread driveThread = new Thread(() -> {
            while (opModeIsActive()) {
                // If you truly want ONLY rotation, uncomment the 3 lines below and comment the mecanum math.
                // double rx = gamepad1.right_stick_x;
                // lf.setPower(rx); lb.setPower(rx); rf.setPower(-rx); rb.setPower(-rx);
                // continue;
        ...
        Thread turretThread = new Thread(() -> {
            ...
            while (opModeIsActive()) {
                ...
                // Always update lastKnownAbs direction whenever we are actually pointing somewhere
                // so HOLD uses current heading correctly even after robot rotates.
                // Also, update it when tag is well-centered.
                ...
                if (manualNow) {
                    ...
                    // Update lastKnownAbs based on current pointing direction (so when you let go, it holds here)
                    lastKnownAbsDeg = angleWrapDeg(robotHeadingDeg + turretRelDeg);
                    haveLastKnown = true;
                ...
                // Not manual => AUTO
                // Compute needed turret rel to face lastKnownAbs using CURRENT heading
                ...
                // Mode selection (with hysteresis) — TRACK wins if tag visible
                TurretMode next;
                if (hasTarget) {
                ...
                if (turretMode == TurretMode.TRACK) {
                    ...
                    // pick correct edge based on sign of needed
                    double desiredEdge = (relUnclampedNeeded >= 0) ? maxTurretDeg : minTurretDeg;

                    // initialize sweep target on entry (use NaN sentinel instead of 0.0)
                    ...
                    cmdPower = 0.0;
                    edgePauseTimer = 0.0;
                    sweepTargetDeg = Double.NaN; // reset sentinel
                }
        ...
        while (opModeIsActive()) {
            ...
            // Turret telemetry (super important)
            ...
            // old distance telemetry (guard null)
